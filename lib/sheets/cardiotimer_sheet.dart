...
class IntervalTimerSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Watch Signals.
    final int initialSeconds = IntervalTimer.sInitialIntervalTime.watch(
      context,
    );
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        // Circular Picker.
        _CircularIntervalPicker(
          seconds: initialSeconds,
          onChanged: (int newSeconds) {
            IntervalTimer.sInitialIntervalTime.value = newSeconds;
            // Also update elapsed time if timer isn't running.
            if (!IntervalTimer.sIntervalTimerRunning.value) {
              IntervalTimer.sElapsedIntervalTime.value = newSeconds;
            }
          },
        ),
        const SizedBox(height: 40),
          ...
          children: <Widget>[
            Expanded(
              child: ElevatedButton(
                onPressed: () {
                  IntervalTimer.sInitialIntervalTime.value = 60;
                  IntervalTimer.sElapsedIntervalTime.value = 60;
...
class _CircularIntervalPicker extends StatelessWidget {
  const _CircularIntervalPicker({
