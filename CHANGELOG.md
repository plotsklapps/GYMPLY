...
- **Exercise Notes Feature:**
  ...
  - Added `ExerciseNoteModal` (`lib/modals/exercisenote_modal.dart`) with unlimited multi-line text input for recording exercise cues, seat settings, or tips.
  - Added a dedicated Note button with a dynamic badge indicator (visible whenever a note is present) to all exercise screens.
- **iOS Support & Background Audio Engine:**
  - Configured iOS `UIBackgroundModes` (`audio`, `fetch`, `processing`) and `AVAudioSessionCategory.playback` in `AudioService` for reliable timer bell playback when the app is minimized or screen is locked.
  - Integrated native iOS scheduled local notifications with custom `timerbell.wav` audio for background alerts.
  - Synchronized foreground/background audio playback to eliminate sound stuttering and truncation.
- **Save Workout UX Improvements:**
  - Refactored the title field in `SaveWorkoutModal` to show the autogenerated day/time title (e.g. *"Friday Evening Workout"*) as `hintText` that disappears on tap/focus.
  - Automatically saves the autogenerated title if the title field is left empty.
  - Enabled sentence capitalization (`TextCapitalization.sentences`) for title and notes text fields.
- **UI Layout & Compact Controls:**
  - Compacted action buttons and choice chips across `StrengthExerciseScreen`, `CardioExerciseScreen`, and `StretchExerciseScreen`.
  - Wrapped control rows in `FittedBox` to prevent UI overflow on narrow mobile screens.
- **Dependency & Build Updates:**
  - Upgraded dependencies including `ndk` and Android build configurations.
